...
/**
 * ChangePasswordBindingModel Class Doc Comment
 *
 * @category Class
 * @package  Ringba
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class ChangePasswordBindingModel implements ModelInterface, ArrayAccess
{
    ...
    /**
      ...
      * @var string
      */
    protected static $swaggerModelName = 'ChangePasswordBindingModel';
    ...
    public function listInvalidProperties()
    {
        ...
        if (!preg_match("/(?=^.{6,}$)(?=.*?\\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^a-zA-Z\\d]).*$/", $this->container['password'])) {
            $invalidProperties[] = "invalid value for 'password', must be conform to the pattern /(?=^.{6,}$)(?=.*?\\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^a-zA-Z\\d]).*$/.";
        }
    ...
    public function setPassword($password)
    {
        if ((mb_strlen($password) > 100)) {
            throw new \InvalidArgumentException('invalid length for $password when calling ChangePasswordBindingModel., must be smaller than or equal to 100.');
        }
        if ((mb_strlen($password) < 6)) {
            throw new \InvalidArgumentException('invalid length for $password when calling ChangePasswordBindingModel., must be bigger than or equal to 6.');
        }
        if ((!preg_match("/(?=^.{6,}$)(?=.*?\\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^a-zA-Z\\d]).*$/", $password))) {
            throw new \InvalidArgumentException("invalid value for $password when calling ChangePasswordBindingModel., must conform to the pattern /(?=^.{6,}$)(?=.*?\\d)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[^a-zA-Z\\d]).*$/.");
        }
